...
public class Java_ByteBuffer {
    ...
    private static void readRawBytes() {
        try (final FileInputStream fis = new FileInputStream(readfile);
             ...
             *  This approach is can be more efficient, especially when you slew over most of the data, or access it
             *  via the backing array.  It will pay off if for example you were processing just a 4-byte field in
             *  a 512-byte record, since only the bytes you need are copied from the buffer, not the entire record.
             */
    ...
    /**
     * created the RandomAccessFile in read-only mode (by passing the flag r to its constructor),
     * and also mapped the channel in read-only mode. This will prevent accidental writes, but more importantly,
     * it means that the file won't count against the program's commit charge.
     * On a 64-bit machine, you can map terabytes of read-only files. And in most cases,
     * you don't need write access: you have a large dataset that you want to process,
     * and don't want to keep reading chunks of it into heap memory.
     */
    private static void readingWithMappedByteBuffer() {
        // reading a file with memoryMapping
        File file = new File("/home/go-along-road-quick-on-time-4-ye/Desktop/goodbooks/JavaScript_Enlightenment.pdf");
        try (FileInputStream fis = new FileInputStream(file);
             FileChannel ch = new RandomAccessFile(file, "r").getChannel();) {

            // FileChannel ch = fis.getChannel();
            // ch.position(file.length() / 200000 - 2);
            MappedByteBuffer mappedbb = ch.map(FileChannel.MapMode.READ_ONLY, file.length() / 200000 - 2, file.length() / 100000);
            mappedbb.load();
            ...
            /**
             * Read-write files require some more thought.
             * The first thing to consider is just how important your writes are.
             * As I noted above, the memory manager doesn't want to constantly write dirty pages to disk.
             * Which means that your changes may remain in memory, unwritten,
             * for a very long time — which will become a problem if the power goes out.
             * To flush dirty pages to disk, call the buffer's force() method.

             mappedbb.putInt(0, 0x87654321);
             mappedbb.force();

             Those two lines of code are actually an anti-pattern:
             you don't want to flush dirty pages after every write,  or you'll make your program IO-bound.
             Instead, take a lesson from database developers, and group your changes into atomic units
             (or better, if you're planning on a lot of updates, use a real database).

             Few programmers think about what happens to their files when the power goes out.
             Those that do typically stop thinking once they've called flush().
             However, even if you've flushed the writes out of the operating system and into the disk drive,
             they may not have found their way to the disk platter: disk drives generally have an on-drive RAM buffer,
             and blocks live in that buffer until the drive can write them to the physical disk — or the power goes out.
             You can typically tweak the drive's settings via the OS (not Java),
             so if you absolutely, positively must ensure writes, you should learn how your particular drives work.
             */
 ...
 * writing a few tens of kilobytes of data via the usual read and write methods.
 ...
 * <p>
 * Depending on your filesystem, you can create files larger than 2GB.
 * But if you look at the ByteBuffer documentation, you'll see that it uses an int for all indexes,
 * which means that buffers are limited to 2GB.
 * Which in turn means that you need to create multiple buffers to work with large files.
 * <p>
 * One solution is to create those buffers as needed.
 * The same underlying FileChannel can support as many buffers as you can create, limited only by the OS and available virtual memory;
 * simply pass a different starting offset each time.
 * The problem with this approach is
 * that creating a mapping is expensive, because
 * 1> it's a kernel call (and you're using mapped files to avoid kernel calls).
 * 2>  a page table full of mappings will mean more expensive context switches.
 * As a result, as-needed buffers aren't a good approach unless you can divide the file into large chunks that are processed as a unit.
 * <p>
 * Enabling Large Direct Buffers AND Thread Safety:
 * http://www.kdgregory.com/index.php?page=java.byteBuffer
 */
class Java_MappedByteBuffer {
    ...
    /**
     * segmentSize:
     * Integer.MAX_VALUE is the maximum index value for a buffer.
     * While that would result in the fewest number of buffers to cover the file,
     * and you won't be able to access multi-byte values at segment boundaries.
     * <p>
     * The segment size is Integer.MAX_VALUE / 2,  each buffer is twice that size
     */
    public void MappedFileBuffer(File file, int segmentSize, boolean isReadWrite) {
        if (segmentSize > Integer.MIN_VALUE)
            throw new IllegalArgumentException("segment size too large (max " + Integer.MIN_VALUE + "): " + segmentSize);
