...
public class Leetcode811SubdomainVisitCount {
  /*
  ...
  */
  /*
  O(N) time and space,
       N is the length of cpdomains,
       assuming the length of cpdomains[i] is fixed.
    */
  public List<String> subdomainVisits(String[] cpdomains) {
    Map<String, Integer> map = new HashMap();
    for (String e : cpdomains) {
      String[] parts2 = e.split("\\s+");
      String[] ds = parts2[1].split("\\.");
      int count = Integer.valueOf(parts2[0]);
      String domain = "";
      for (int i = ds.length - 1; i >= 0; i--) {
        domain = ds[i] + (i < ds.length - 1 ? "." : "") + domain;
        map.put(domain, map.getOrDefault(domain, 0) + count);
      }
    }

    List<String> ans = new ArrayList();
    for (String d : map.keySet()) ans.add("" + map.get(d) + " " + d);
    return ans;
  }
}
